...
module.exports = exports = function fts (schema, options) {
 ...
 /**
  ...
  schema.methods.metaKeywords = function () {
    ...
    while (i--) {
      var keywords = stemmer.metaphoneString(values[i] || '');

      keywords.forEach(function (mKey) {
        if (mKey) keywordSet[mKey] = true;
      });
    }

    return Object.keys(keywordSet);
  };
      ...
      stemmed.forEach(function (stem) {
        if (stem) {
          if (upper)
            ...
            keywordSet[stem.toLowerCase()] = true;
        }
      });
    }
      ...
      words.forEach(function (word) {
        if (word) {
          if (upper)
            ...
            keywordSet[word.toLowerCase()] = true;
        }
      });
    }
